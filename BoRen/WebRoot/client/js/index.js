...
var indexT = new Vue({
	...
	methods : {
		...
		showMessage:function(){
			var messageParam = "userInfoByMsgSendToId.account.acctId="+all.acctId;
		...
		ssSend:function(type){
			...
			"&userInfoByEmailSendId.account.acctId="+all.acctId+
			"&emailType.emailTypeId="+type+
			"&emailType_1=1";
...
$(function () {
	$("body").on("click",".scroll_colors",function() {
		...
		if ($(this).text() == "签到") {
			var sign ="account.acctId="+all.acctId;
		...
		if($(this).text()=="系统通知"){
             $(".xuhua_bj1").show();
             var noticeParam ="userInfoByEmailSendToId.account.acctId="+all.acctId;
