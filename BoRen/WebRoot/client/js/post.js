...
var postT = new Vue({
	...
	methods:{
		...
		postShow:function(){
			/*
			if(param.indexOf("acctId=")!=-1)
				allParam.acctId = param.substring(param.indexOf("acctId=")+7);
			}*/
			...
			$.getJSON("../postQueryOneById.do",postParam,function(data){
				allParam.louZhuId = data["postList"][0].acctId;
				allParam.postInfo=data["postList"][0];
				
				//楼层参数
				var cmmtParam="post.postId="+allParam.postId+
		...
		cmmtTwoShow:function(cmmtId,currentPage,index,type,sendToId){
			  if (type == 1) {
				...
				if(currentPage==undefined)
					currentPage=1;
				var cmmtParam = "comment.cmmtId=" + cmmtId + 
		...
		postB:function(){
			...
			$.getJSON("../postTouB.do", bParam, function(data) {
				if(data["B"]!=-1){
					allParam.postInfo.postCoinNum = data["B"];
					alert("投币成功");
				}else{
					alert("余额不足");
				}
		...
		postLove:function(){
			...
			if(cki==undefined||cki==null)
				...
				if(result==0){
					...
					"&sendId="+allParam.acctId+
					"&sendToId="+allParam.postInfo.acctId;
		...
		sendComment:function(){
			sendCmmtParam = "post.postId="+allParam.postId+"&cmmtText="+allParam.sendCmmtText+
			"&userInfoByMsgSendId.account.acctId="+allParam.acctId+
			"&userInfoByMsgSendToId.account.acctId="+allParam.louZhuId
			$.getJSON("../commentAdd.do",sendCmmtParam , function(data) {
				alert(data["result"]);
				if(data["result"]=="评论成功"){
					allParam.cmmtCurrentPage
		...
		sendTwo:function(floor,cmmtId){
			...
			sendCmmtParam = "post.postId="+allParam.postId+
			"&cmmtTwoText="+allParam.twoText+
			"&userInfoByMsgSendId.account.acctId="+allParam.acctId+
			"&userInfoByMsgSendToId.account.acctId="+allParam.twoAcctId+
			"&comment.cmmtId="+cmmtId+
			"&cmmtFloor="+floor;
		...
		guanZhu:function(guanZhuId){
			var guanZhuParam = "guanZhuId="+guanZhuId+
			"&fenSiId="+allParam.acctId;
			 ...
			 if(floorIndex==1)
				 ...
				 if(floorIndex>floorLength){
					 currentPage = parseInt(
							 				parseFloat(floorIndex) % parseFloat(4)
							 				?(parseInt(floorIndex)/parseInt(4)+1)
							 				:(parseInt(floorIndex)/parseInt(4)+1)
						 					);
