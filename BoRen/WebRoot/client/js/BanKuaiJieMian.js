...
var postType = new Vue({
	...
	methods:{
		...
		sendPost:function(){
			...
			"&postType.postTypeId="+all.sendPostTypeId+
			"&userInfo.account.acctId="+all.acctId;
		 ...
		 $(window).scroll(function(e) {
			 if ($(document).scrollTop() <= 0) {
			  ...
			  if ($(document).scrollTop() >= $(document).height() - $(window).height()) {
				  paramPost.currentPage=paramPost.currentPage+1;
				/* param = "currentPage="+paramPost.currentPage+
				 "&postType.postTypeId="+all.sendPostTypeId;*/
				 setTimeout(function() {
					 if (lastTime - e.timeStamp == 0) {
						/* $.getJSON("../postTypeQuery.do",param, function(data) {
							 all.postList =  data["postList"];
							 paramPost.currentPage = data["currentPage"];
						 });*/
						 postType.showPost();
